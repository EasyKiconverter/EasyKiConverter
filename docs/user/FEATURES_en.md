...
Create KiCad (.kicad_mod), Altium (.PcbLib), Xpedition ASCII, PADS ASCII (Schematic Decal, Part Type, and PCB Decal), P-CAD ASCII (.lia), or Eagle XML (.lbr) libraries from EasyEDA data. Eagle can write symbols, packages, and device associations together; 3D models are emitted by an independent stage.

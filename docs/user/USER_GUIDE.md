...
缓存目录必须是空目录，或已经包含 EasyKiConverter 创建的所有权标记；文件系统根目录、用户主目录及符号链接目录会被拒绝。编辑缓存路径时不会在每个字符输入后执行迁移，只有选择目录或编辑完成并通过校验后才会应用。

缓存维护遵循以下边界：

- 根目录和缓存条目必须通过所有权标记及元数据身份校验；未知文件、目录、符号链接和无法验证的旧缓存会保留。

...
- Xpedition: IR-to-symbol and Pads/Cell HKP ZIP exporters exist. Pad, Hole, Padstack, Cell, Pin, Outline, and PDB associations parse into format-specific models; Cell outlines map to `FootprintOutlineIR`. V54 symbol text now parses pins, graphics, text, and multi-part metadata and maps them to the existing Symbol IR through an adapter. The HKP adapter accepts a caller-provided symbol index and can also compose multiple parsed HKP and symbol documents, aggregating Cell, Padstack, PDB, and symbol associations into ComponentIR. Cross-file duplicate symbols, missing associations, and ambiguous associations produce diagnostics instead of fabricated components.
- Cadstar ASCII libraries: Pad, Package, Component, Part, pin-number ranges, custom pads, slots, basic graphics, unit conversion, and Footprint/Symbol IR adapters are implemented; valid Part associations are aggregated into ComponentIR, and the adapter result no longer exposes the Cadstar-specific Part model. Missing or ambiguous associations do not produce fabricated components.

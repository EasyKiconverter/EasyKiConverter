...
- 代码中存在相关实现入口：EasyEDA/LCSC 数据导入、统一 IR，以及 KiCad、Altium、Xpedition、Cadstar、P-CAD、PADS、Eagle、OrCAD、Allegro 等目标的部分代码路径。这些入口不自动等同于可发布支持。
- 进行中：持续完善格式解析、目标格式映射、跨平台打包和缓存安全等能力。
- 计划支持：未在源码和测试中形成完整链路的格式或商业软件原生验证，不得在贡献说明中当作已支持。

对任何格式声明“支持”之前，必须同时检查：对应 Importer/Exporter 实现、自动化测试、格式文档中的能力状态，以及是否存在商业 EDA 实机验证记录。存在导出器类不等于完成了目标 EDA 的商业软件兼容性验证；结构测试、自动化测试和实机验证必须分别报告。
...
当源码、文档、Issue 或计划说明不一致时，按以下顺序判断：

1. 当前分支的源码和测试。
2. 当前 workflow 配置和对应 CI 实际结果。
3. 当前格式、架构和构建文档。
4. Issue/PR 中已经确认的复现和结论。
5. Roadmap 或其他计划性说明。

低优先级内容不能覆盖高优先级事实；审查或实现报告必须标明结论对应的分支和提交。

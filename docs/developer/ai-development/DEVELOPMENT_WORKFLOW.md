...
保留用户已有修改，不使用破坏性 reset 或 checkout 覆盖它们。一个相对完整的问题或主题对应一个工作分支；同一分支可以包含多个相关提交，不要为每个小改动、子任务或单个提交创建新分支。工作分支必须从该问题所属的版本分支创建，完成后 PR 回到创建它时使用的同一版本分支，不直接在版本分支上提交。仓库没有发现强制分支命名格式，命名示例仅供识别。

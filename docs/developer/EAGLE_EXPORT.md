...
当前支持 **EasyEDA/LCSC → Eagle XML `.lbr` 库**，可在同一文件中写入封装、符号、DeviceSet 和引脚到焊盘的关联。三维模型仍由独立模型阶段输出；由于 Eagle `package3d` 依赖受管 URN，本项目不会伪造托管关联。
...
- 符号曲线、圆弧和无法由 Eagle XML 基础图元表达的几何会拒绝导出，而不是静默降级。

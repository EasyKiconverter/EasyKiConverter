...
另一个公开库 `taoyilee/PCB_LIB` 提供了更多样本：`0805r.dra` 的前四字节为 `03 0c 13 00`，`142-0771-821.dra` 和对应 `.psm` 的前四字节为 `03 10 13 00`，而对应 `.pad` 为 `03 02 13 00`。这些样本再次表明，即使文件来自同一公开库，不同文件角色也有不同头部布局；版本和角色不能靠扩展名推断。

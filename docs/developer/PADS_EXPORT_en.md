...
The current feature supports **EasyEDA/LCSC to PADS Parts Library ASCII Schematic Decal and PCB Decal** export, with independent WRL/STEP model output. It is not a PADS native binary library or a complete Part Type component-library exporter.
...
flowchart LR
    ...
    C --> PADS[User PADS library workflow]
    D --> PADS
    Models --> PADS
```
...
- The symbol stage writes PADS ASCII Schematic Decals (`.c`) with basic graphics, text, pins, and multipart splitting.
...
- The `.c` output is a Schematic Decal graphics library; it does not include Part Type (`.p`) device records, footprint associations, or pin mapping. Users must complete those associations in PADS.
...
Import or associate the generated `.c`, `.d`, and `.3dmodels` outputs through the library workflow of the target PADS version. Confirm layer semantics, text encoding, manufacturing attributes, and Part Type pin mapping in that software; this project does not claim cross-version PADS native compatibility.

## Tests

`tests/unit/test_pads_exporter.cpp` uses a local IR fixture to verify multipart Schematic Decals, pins, basic graphics, end records, the PCB Decal mil unit header, SMD/PTH pad stacks, sanitized-name collisions, and failure diagnostics for data that cannot be represented losslessly. The tests do not access the network or require a local PADS installation.

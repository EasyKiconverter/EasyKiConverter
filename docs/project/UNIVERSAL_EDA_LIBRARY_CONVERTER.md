...
独立三维模型默认写入 `<库名称>.3dmodels/` 目录，并根据模型名称生成文件名；同名模型会自动追加稳定后缀，避免不同元件互相覆盖。启用“不覆盖”时，已有的全部目标文件会被跳过，若同一模型只存在部分目标格式文件则该元件报告失败，保留已有文件不变。

公共 3D 阶段还会在该目录生成 `manifest.json`。它记录组件 ID、符号名称、封装名称、模型文件、模型 UUID、位移、旋转、STEP 偏移和逐项状态，用于把独立三维文件与同一次导出的符号库、封装库对应起来。该文件是 EasyKiConverter 的项目级清单，不代表目标 EDA 已建立原生 3D 关联。
